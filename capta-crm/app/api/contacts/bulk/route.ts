...
export async function POST(request: Request) {
  ...
  let duplicates = 0; const now = new Date();
  const records = normalized.filter((contact) => { if (contact.email && knownEmails.has(contact.email)) { duplicates++; return false; } if (contact.email) knownEmails.add(contact.email); return true; }).map((contact) => ({ id: crypto.randomUUID(), listId: body.listId!, ...contact, source: 'import', createdAt: now, updatedAt: now }));
