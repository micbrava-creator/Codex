...
import { defaultLeadConfirmation } from '../../../lib/lead-confirmation-email';

export async function GET() {
  if (!await requireApiUser()) return NextResponse.json({ error: 'Não autorizado' }, { status: 401 });
  const supabase = await createSupabaseServerClient();
  const { data } = await supabase.from('crm_email_settings').select('*').eq('id', defaultLeadConfirmation.id).maybeSingle();
  return NextResponse.json(data || defaultLeadConfirmation);
}
export async function PATCH(request: Request) {
  ...
  const supabase = await createSupabaseServerClient();
  const { error } = await supabase.from('crm_email_settings').upsert({ id: defaultLeadConfirmation.id, subject, message, updated_by: manager.memberId, updated_at: new Date().toISOString() });
  if (error) return NextResponse.json({ error: 'Não foi possível salvar' }, { status: 500 });
  return NextResponse.json({ ...defaultLeadConfirmation, subject, message });
}
