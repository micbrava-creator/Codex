...
import { cardTasks, contactLists, contacts, pipelines, pipelineStages } from '../../../db/schema';
...
export async function GET() {
  ...
  const [allPipelines, stages, lists] = await Promise.all([getDb().select().from(pipelines).orderBy(desc(pipelines.createdAt)), getDb().select().from(pipelineStages).orderBy(asc(pipelineStages.position)), getDb().select({ id: contactLists.id, name: contactLists.name, pipelineId: contactLists.pipelineId }).from(contactLists)]);
  ...
  return NextResponse.json(allPipelines.map((pipeline) => ({ ...pipeline, lists: lists.filter((list) => list.pipelineId === pipeline.id), stages: stages.filter((stage) => stage.pipelineId === pipeline.id).map((stage) => ({ ...stage, cards: cards.filter((card) => card.stageId === stage.id).map((card) => ({ ...card, tasks: tasks.filter((task) => task.contactId === card.id) })) })) })));
}
