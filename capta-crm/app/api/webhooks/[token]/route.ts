...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const now = new Date(); const record = { id: crypto.randomUUID(), listId: list.id, name, email, phone, company, notes: value(body, ['notes', 'observacoes', 'mensagem']), source: 'webhook', createdAt: now, updatedAt: now }; await getDb().insert(contacts).values(record); return NextResponse.json({ ok: true, contactId: record.id }, { status: 201 });
}
