...
export function contactFromDb(row: Record<string, any>) {
  return { id: row.id, listId: row.list_id, productId: row.product_id ?? null, name: row.name, email: row.email, phone: row.phone, company: row.company, notes: row.notes, source: row.source, createdAt: row.created_at, updatedAt: row.updated_at, stageId: row.stage_id, assignedUserId: row.assigned_user_id, negotiationValueCents: row.negotiation_value_cents, saleCompleted: row.sale_completed, saleCompletedAt: row.sale_completed_at, followUpEnabled: row.follow_up_enabled ?? true, followUpIntervalDays: row.follow_up_interval_days ?? 2, followUpIntervalMinutes: row.follow_up_interval_minutes ?? (row.follow_up_interval_days ?? 2) * 1440 };
}
export function listFromDb(row: Record<string, any>) {
  return { id: row.id, name: row.name, segment: row.segment, color: row.color, webhookToken: row.webhook_token, createdAt: row.created_at, pipelineId: row.pipeline_id, routingStageId: row.routing_stage_id, assignmentMode: row.assignment_mode, fixedSellerId: row.fixed_seller_id, rotationCursor: row.rotation_cursor, emailAlertsEnabled: row.email_alerts_enabled, confirmationEmailEnabled: row.confirmation_email_enabled, followUpEnabled: row.follow_up_enabled ?? true, firstFollowUpDelayMinutes: row.first_follow_up_delay_minutes ?? 15, followUpIntervalMinutes: row.follow_up_interval_minutes ?? 2880, followUpTitle: row.follow_up_title ?? 'Entrar em contato com {{lead}}', followUpNotes: row.follow_up_notes ?? '', nextFollowUpTitle: row.next_follow_up_title ?? 'Retomar contato com {{lead}}', nextFollowUpNotes: row.next_follow_up_notes ?? '' };
}
...
export function taskFromDb(row: Record<string, any>) { return { id: row.id, contactId: row.contact_id, title: row.title, dueDate: row.due_date, completed: row.completed, kind: row.kind ?? 'task', notes: row.notes ?? '', reminderEnabled: row.reminder_enabled ?? false, reminderSentAt: row.reminder_sent_at, createdAt: row.created_at, updatedAt: row.updated_at }; }
